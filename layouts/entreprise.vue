...
const { profile, user, logout, isGerant, enterprise } = useAuth()

import { useNotificationsStore } from '~/stores/notifications'
const notificationsStore = useNotificationsStore()

// État pour les requêtes de plan
const trialRequest = ref(null)
const pendingRequest = ref(null)
const hasActiveTrial = computed(() => trialRequest.value !== null)

// Plan actuel (avec essai si actif)
...
onMounted(async () => {
    if (user.value?.uid) {
        ...
        // Charger les requêtes de plan
        try {
            const { getPendingPlanChangeRequests } = await import('~/firebase/services/firestore')
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-6">
                ...
                <div>
                    <div v-if="!sidebarCollapsed" class="px-3 pt-4 pb-3">
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">MISSIONS</p>
                    </div>
                    <div class="space-y-1">
                ...
                <!-- Audits Section (gérant only) -->
                <div v-if="isGerant && navigation.audits">
                    <div v-if="!sidebarCollapsed" class="px-3 pt-4 pb-3">
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">AUDITS</p>
                    </div>
                    <div class="space-y-1">
                ...
                <div v-if="navigation.membres">
                    <div v-if="!sidebarCollapsed" class="px-3 pt-4 pb-3">
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">ÉQUIPE</p>
                    </div>
                    <div class="space-y-1">
                ...
                <div>
                    <div v-if="!sidebarCollapsed" class="px-3 pt-4 pb-3">
                        <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">COMPTE</p>
                    </div>
                    <div class="space-y-1">
                ...
                <!-- Plan Badge -->
                <div class="mt-8 px-3">
                    <BadgePlan v-if="enterprise" :plan="currentPlan" :collapsed="sidebarCollapsed" />
                    <!-- Indication si essai -->
                    <div v-if="hasActiveTrial" class="mt-2 px-3">
                        <div class="text-xs text-amber-600 font-medium text-center">
                            <Icon name="heroicons:clock" class="w-3 h-3 inline mr-1" />
                            Essai {{ trialRequest?.requestedPlan }} actif
                        </div>
                    </div>
                </div>
            </nav>
