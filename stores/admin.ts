...
export const useAdminStore = defineStore('admin', {
    ...
    getters: {
        ...
        filteredUsers: (state): UserWithDetails[] => {
            return state.users.filter(user => {
                ...
                // Text search: email, name
                if (state.userNameEmailQuery.trim()) {
                    const q = state.userNameEmailQuery.toLowerCase()
                ...
                // Company name search
                if (state.userCompanyQuery.trim()) {
                    const q = state.userCompanyQuery.toLowerCase()
                    const enterpriseName = (user.enterpriseProfile?.companyName || '').toLowerCase()
                    
                    // Chercher aussi dans le nom de l'entreprise parente pour les membres
                    let parentEnterpriseName = ''
                    if (user.enterpriseOwnerId && !enterpriseName) {
                        const parentEnterprise = state.users.find(u => 
                            u.uid === user.enterpriseOwnerId && u.enterpriseProfile?.companyName
                        )
                        parentEnterpriseName = (parentEnterprise?.enterpriseProfile?.companyName || '').toLowerCase()
                    }

                    if (!enterpriseName.includes(q) && !parentEnterpriseName.includes(q)) {
                        return false
                    }
                }
    ...
    actions: {
        ...
        /**
         * Reject a user registration (change status to 'deleted')
         */
        ...
        /**
         * Delete a user account (change status to 'deleted')
         */
        async deleteUser(uid: string): Promise<boolean> {
            try {
                ...
                this.usersError = 'Erreur lors de la suppression du compte'
                return false
            }
        },
        ...
        /**
         * Set text search query for users (legacy)
         */
        ...
        /**
         * Set name/email search query
         */
        setUserNameEmailQuery(query: string): void {
            this.userNameEmailQuery = query
        },

        /**
         * Set company name search query
         */
        setUserCompanyQuery(query: string): void {
            this.userCompanyQuery = query
        },
