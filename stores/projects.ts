...
export const useProjectsStore = defineStore('projects', {
    ...
    getters: {
        ...
        remainingProjects: (state) => {
            if (!state.enterpriseProfile) return 0
            const maxProjects = PLAN_LIMITS[state.enterpriseProfile.plan || 'free'].maxProjects
            return Math.max(0, maxProjects - state.projects.length)
        },

        /** Projects count string (e.g., "1/3") */
        projectsCountString: (state) => {
            if (!state.enterpriseProfile) return "0/1"
            const maxProjects = PLAN_LIMITS[state.enterpriseProfile.plan || 'free'].maxProjects
    ...
    actions: {
        ...
        async fetchProjects(enterpriseId: string): Promise<void> {
            ...
            try {
                ...
                console.log('🔍 Checking canCreateMore for enterprise:', enterpriseId)
                this.canCreateMore = await canCreateProject(enterpriseId)
                console.log('🔍 canCreateMore result:', this.canCreateMore)
