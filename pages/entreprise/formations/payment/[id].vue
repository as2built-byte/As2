...
/**
 * Enterprise Dahabia Payment Page
 */
...
async function processPayment() {
    ...
    try {
        ...
        setTimeout(() => {
            router.push('/entreprise/formations')
        }, 3000)
    } catch (err) {
...
async function loadFormation() {
    ...
    try {
        const formations = await getFormationsWithStatus(user.value.uid, 'enterprise')
        ...
        if (!formation.value) {
            ...
            router.push(`/entreprise/formations/${formationId.value}`)
        }
    } catch (err) {
...
<template>
    <div class="max-w-2xl mx-auto">
        <!-- Back Button -->
        <NuxtLink 
            :to="`/entreprise/formations/${formationId}`"
